...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // 4. Telepon (format 08xxxxxxxxxx, 10-13 digit)
    if (empty($telepon)) {
    ...
    // Cek kode anggota & email duplikat
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT id_anggota FROM anggota WHERE kode_anggota = ? OR email = ?");
        $stmt->bind_param("ss", $kode_anggota, $email);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows > 0) {
            $row_cek = $result->fetch_assoc();
            $errors[] = "Kode anggota atau email sudah digunakan";
        }
        $stmt->close();
    }
    ...
    $foto = '';
    if (!empty($errors) === false && isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
    ...
    if (count($errors) == 0) {
        ...
        if ($stmt->execute()) {
            $stmt->close();
            closeConnection();
            header("Location: index.php?pesan=" . urlencode("Anggota '$nama' berhasil ditambahkan") . "&tipe=sukses");
            exit();
        } else {
